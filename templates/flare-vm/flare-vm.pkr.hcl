...
locals {
  template_description = "Flare VM (Windows 11 22H2 64-bit Enterprise) template built ${legacy_isotime("2006-01-02 03:04:05")} username:password => localuser:password"
}

source "proxmox-iso" "flare-vm" {
...
build {
  sources = ["source.proxmox-iso.flare-vm"]
  ...
  provisioner "ansible" {
    playbook_file = "ansible/install-flare.yml"
    use_proxy     = false
