...
import EnterpriseSchema from "@site/static/schemas/range-config.enterprise.json";
import JSONSchemaViewer from "@theme/JSONSchemaViewer";

<Tabs groupId="edition">
  <TabItem value="oss" label="🏟️ Open Source">
...
<JSONSchemaViewer schema={ Schema } />
  ...
  <TabItem value="enterprise" label="🏛️ Enterprise">
...
router:
  ...
  template: debian11-x64-template
  ram_gb: 2
  ram_min_gb: 0.5
  cpus: 2
  roles:
  ...
  role_vars:
    somevar: value
  outbound_wireguard_config: |- # NOTE: this config must have AllowedIPs set to 0.0.0.0/0, split tunnels are not supported (yet)
    [Interface]
    PrivateKey = XXXX
    ...
  outbound_wireguard_vlans: # Define the VLANs that the router VM will route traffic for. This key is required if outbound_wireguard_config is defined.
    - 10
...
<JSONSchemaViewer schema={ EnterpriseSchema } />
  </TabItem>
