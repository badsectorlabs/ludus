...
router:
  ...
  role_vars:
    somevar: value
  outbound_wireguard_config: |- # NOTE: this config must not set DNS and must exclude 198.51.100.0/24, 10.0.0.0/8, 192.0.2.0/24, and the address of the WireGuard server
    [Interface]
    PrivateKey = XXXX
    ...
