...
ludus:
  - vm_name: "{{ range_id }}-ad-dc-win2019-server-x64" # The name of the VM in Proxmox. You can use the `{{ range_id }}` template string which resolves to your range ID (i.e. JS)
    ...
    dns_rewrites:           # Any values in this array will be added to DNS for the range and return an A record for this VM's IP
      ...
      - '*.example.com'     # rewrites responses for example.com and all example.com subdomains
    ...
    target_node: pve1             # The proxmox node to deploy the range to (cluster mode only); overrides the defaults.target_node value
...
# Store data for external programs under this key. Ludus does not interpret its contents.
ludus_extensions:
  asset_tracker:
    project_id: training-range
    labels:
      - windows
      - active-directory
...
router:
  ...
  ram_min_gb: 1
  cpus: 2
  upstream_dns: # DNS resolvers used by the router. Entries can be bare IPs or DNS-over-HTTPS URLs. Default: https://1.1.1.1/dns-query
    - 1.1.1.1
  ...
  target_node: pve1  # The proxmox node to deploy the router to (cluster mode only); overrides the defaults.target_node value
...
network: 
  ...
  wireguard_vlan_default: ACCEPT # The default rule to apply to traffic leaving the range to clients connected via WireGuard. Default: REJECT
  ...
  rules: # Specify rules to restrict or allow based on inter_vlan_default. Default: allow all traffic
    - name: Only allow windows to kali on 443 # The rule name will be added as a comment in iptables
      ...
      ports: 443     # A single port, a range in the format start:end, a comma-separated list, or 'all'
      action: ACCEPT # ACCEPT|REJECT|DROP
    - name: Allow multiple TCP ports from VLAN 10 to VLAN 20
      vlan_src: 10
      vlan_dst: 20
      protocol: tcp
      ports: 8080, 8090, 4433
      action: ACCEPT
...
defaults:
  ...
  target_node: pve1                        # The proxmox node to deploy the range to (cluster mode only) if a vm object does not have it's own target_node value. Leave empty for Ludus to decide the node for each VM based on resource utilization
...
Unknown top-level keys are rejected so that misspelled Ludus settings cannot pass validation. Put application-specific data under `ludus_extensions` instead. The value must be a mapping, but each program's entry may contain any JSON-compatible YAML value. Give each program its own key:
...
ludus_extensions:
  asset_tracker:
    ...
    labels:
      - windows
      - active-directory
  example_com_reporting:
    retention_days: 30
```

Ludus preserves this data without interpreting it. The complete range config is also passed to Ansible as extra variables, so roles can read values such as `ludus_extensions.asset_tracker.project_id`.
