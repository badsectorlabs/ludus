...
ludus:
  - vm_name: "{{ range_id }}-ad-dc-win2019-server-x64" # The name of the VM in Proxmox. You can use the `{{ range_id }}` template string which resolves to your range ID (i.e. JS)
    ...
    target_node: pve1     # Optional; cluster only. Proxmox node for this VM (overrides range default). See Cluster Support.
...
router:
  ...
  target_node: pve1  # Optional; cluster only. Proxmox node for the router (overrides range default). See Cluster Support.
...
defaults:
  ...
  target_node: pve1                        # Optional: cluster node placement (see Cluster Support). Default Proxmox node for all VMs in this range; omit for auto-selection.
