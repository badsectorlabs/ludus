...
func serve() {
	...
	// TLS material is generated by bootstrap (install.sh / localgen) and its paths
	// are written into config.yml. The server no longer reads /etc/pve since it
	// runs inside an LXC, not on the Proxmox host.
	...
	if !fileExists(certPath) || !fileExists(keyPath) {
		...
		// Fallback: generate a self-signed pair under ludusInstallPath so the
		// server can still come up for diagnostics rather than crash-looping.
