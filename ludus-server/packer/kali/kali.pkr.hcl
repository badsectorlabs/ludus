...
locals {
  ...
  airgapped_iso_file = "${var.iso_storage_pool}:iso/kali-linux-2026.1-installer-netinst-amd64.iso"
}
...
source "proxmox-iso" "kali" {
  boot_command = [
    ...
    "ludus_ca_url=http://${var.packer_http_bind_address}:{{ .HTTPPort }}/ludus-injected-ca.crt <wait>",
    "<enter><wait>"
  ]
