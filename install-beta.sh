...
#===============================================================================
#
#          FILE: install-beta.sh
# 
#         USAGE: curl https://ludus.cloud/install-beta | bash
#                curl https://ludus.cloud/install-beta | zsh
#                 OR
#                wget -qO- https://ludus.cloud.install-beta | bash
#                wget -qO- https://ludus.cloud.install-beta | zsh
# 
...
# DEFAULTS
#-------------------------------------------------------------------------------
# R2 bucket configuration - bucket has public access
...
print_help() {
  ...
  help_header="Ludus Beta Installer Script"
  help_message="Usage:
  -p INSTALL_PREFIX
...
main() {
  ...
  # Get latest tag from R2 bucket
  if command_exists curl; then
    LATEST_TAG=$(curl -fsSL "${R2_BUCKET_BASE_URL}/latest.txt" | tr -d '\n\r ')
    if [[ -z "${LATEST_TAG}" ]]; then
      print_message "[+] Failed to get latest tag from R2 bucket" "error"
      exit 1
    fi
  elif command_exists wget; then
    LATEST_TAG=$(wget -qO- "${R2_BUCKET_BASE_URL}/latest.txt" | tr -d '\n\r ')
    if [[ -z "${LATEST_TAG}" ]]; then
      print_message "[+] Failed to get latest tag from R2 bucket" "error"
      exit 1
    fi
  ...
  print_message "[+] Latest tag: ${LATEST_TAG}" "info"
  ...
  # Files in R2 bucket are stored without tag suffix
      ...
      if [[ "${ID}" == "debian" ]] && { [[ "${VERSION_ID}" == "12" ]] || [[ "${VERSION_ID}" == "13" ]]; }; then
        ...
        case "${install_server}" in
          "y" ) print_message "[+] Installing Ludus server" "info"
                ...
                # Download from R2 bucket (file stored as ludus-server without tag)
                download_file "${ludus_base_url}/ludus-server" "${tmpdir}" "ludus-server-${LATEST_TAG}"
                download_file_rcode="${?}"
                if [[ "${download_file_rcode}" == "0" ]]; then
